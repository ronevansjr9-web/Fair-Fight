...
export function ProGate({ feature, caseId, children }: ProGateProps) {
  ...
  if (showUpgrade) {
    return (
      <div className="rounded-2xl border border-gold/20 bg-navy p-8 text-center">
        ...
        <p className="mb-6 text-white/70">
          Upgrade to Fair Fight Pro to access {feature.toLowerCase()}. One-time payment of $99 per case — unlimited usage.
        </p>
        <button
          onClick={handleUpgrade}
          disabled={isLoading}
          className="gold-gradient inline-flex items-center rounded-full px-8 py-3 font-semibold text-navy transition-all hover:shadow-lg hover:shadow-gold/20 disabled:opacity-50"
        >
          {isLoading ? "Redirecting..." : "Upgrade to Pro — $99"}
        </button>
  ...
  return (
    <div className="rounded-2xl border border-gold/20 bg-navy p-8 text-center">
      ...
      <p className="mb-6 text-white/70">
        Sign in to access {feature.toLowerCase()} and all Fair Fight features.
      </p>
      <a
        href="/?signin=true"
        className="gold-gradient inline-flex items-center rounded-full px-8 py-3 font-semibold text-navy transition-all hover:shadow-lg hover:shadow-gold/20"
      >
        Sign In to Continue
      </a>
    </div>
