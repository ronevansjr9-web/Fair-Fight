...
import { R as RESTRICTED_FEATURES } from "./restrictedFeatures-Dp1tXKpY.js";
...
function parseAnalysisCaseId(data) {
  const d = data ?? {};
...
function parseRunAnalysisInput(data) {
  const d = data ?? {};
  if (typeof d.caseId !== "string" || !CASE_ID_PATTERN.test(d.caseId)) throw new Error("Invalid case id");
...
const runAnalysis = createServerFn({
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Please review your input and try again."
    };
  }
  try {
