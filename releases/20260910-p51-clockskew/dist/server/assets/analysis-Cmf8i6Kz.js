...
import { b as Route, c as createSsrRpc, t as trackEvent, A as AnalyticsEvents } from "./router-CxY3nyJJ.js";
...
import { s as shouldFetchForSignedInUser } from "./caseFetchGate-Din3Y7TF.js";
...
function AnalysisPage() {
  ...
  const refresh = () => {
    ...
    }).then(applyStatus).catch(() => setStatus({
      status: "error",
      reason: "unavailable"
    }));
  };
  useEffect(() => {
    ...
    fetchAuthedData({
      ...
      fetch: () => getAnalysisStatus({
        data: {
          caseId: search.caseId
        }
      }),
      isUnauthorized: (result) => !("restricted" in result) && !result.ok && result.reason === "unauthorized"
    }).then((outcome) => {
